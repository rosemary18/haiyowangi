...
class _DiscountsViewState extends State<DiscountsView> {
  ...
  void handlerLoadmore() async {
    ...
    if (res.statusCode == 200) {  
      for (var item in res.data!["data"]!["discounts"]) {
        discounts.add(DiscountModel.fromJson(item));
      }
  ...
  Future<void> deleteData(String id) async {
    ...
    if (response.statusCode == 200) {
      scaffoldMessengerKey.currentState?.showSnackBar(
        const SnackBar(
          content: Text("Diskon berhasil di hapus!"),
          backgroundColor: Colors.green,
        ),
      );
      handlerGetData();
      setState(() {
        discounts.removeWhere((element) => element.id.toString() == id);
      });
    } 
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("Apakah anda yakin ingin menghapus diskon ${discounts[i].code}?"),
          ...
          actions: [
            ...
            TouchableOpacity(
              onPress: () async {
                ...
                deleteData(discounts[i].id.toString());
              },
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
  ...
  Widget viewCard(BuildContext context, int index) {
    ...
    if (!loading && !loadmore && discounts.isEmpty)  {
      return const SizedBox(
        height: 40,
        child: Center(
          child: Text("Diskon tidak ditemukan!", style: TextStyle(color: greyTextColor, fontSize: 12)),
        ),
      );
    } 

    if (index == discounts.length && loadmore)  {
      return Center(
        child: Padding(
    ...
    return TouchableOpacity(
      onPress: () async {
        var result = await context.pushNamed(appRoutes.detailDiscount.name, extra: jsonEncode(discounts[index]));
        if (result != null) {
          handlerGetData();
        }
      },
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("#${discounts[index].code ?? "-"}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                  ],
                ),
                TouchableOpacity(
                  child: const Icon(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text(discounts[index].name ?? "-", style: const TextStyle(fontSize: 8))
                    ],
                  ),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text("Berlaku dari", style: TextStyle(fontSize: 8)),
                      Text(formatDateFromString("${discounts[index].dateValid}", format: "EEEE, dd/MM/yyyy"), style: const TextStyle(fontSize: 8)),
                    ],
                  ),
                  const Divider(color: greyLightColor),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text("Berlaku sampai", style: TextStyle(fontSize: 8)),
                      Text(formatDateFromString("${discounts[index].validUntil}", format: "EEEE, dd/MM/yyyy"), style: const TextStyle(fontSize: 8)),
                    ],
                  ),
                ],
              ),
      ...
      body: Column(
        ...
        children: [
          Padding(
            ...
            child: Text("Daftar Diskon ($total)", style: const TextStyle(fontSize: 12)),
          ),
          Expanded(
            child: Container(
              ...
              child: RefreshIndicator(
                ...
                child: ListView.builder(
                  ...
                  itemCount: loading ? 3 : loadmore ? discounts.length + 1 : discounts.isNotEmpty ? discounts.length : 1,
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                TouchableOpacity(
                  onPress: () async {
                    var result = await context.pushNamed(appRoutes.formDiscount.name);
                    if (result != null) {
                      handlerGetData();
                    }
                  },
