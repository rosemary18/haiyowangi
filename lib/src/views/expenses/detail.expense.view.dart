...
class _DetailExpenseViewState extends State<DetailExpenseView> {
  ...
  void initState() {
    super.initState();
    _expense = ExpenseModel.fromJson(jsonDecode(widget.data));
    handlerGetDetail();
    handlerGetTags();
    setState(() {});
  }
  ...
  Future<void> handlerGetDetail() async {
    ...
    Response response = await repository.getDetail("${_expense.id}");
    debugPrint(response.data.toString());
    if (response.statusCode == 200) {
      _expense = ExpenseModel.fromJson(response.data!["data"]);
      setState(() {});
    } else {
  ...
  void handlerEdit() {
    _controllerTag.text = _expense.tag!.toString();
    _controllerName.text = _expense.name!;
    _controllerDate.text = _expense.createdAt!;
    _controllerNominal.text = _expense.nominal!.toString();
    _controllerDescription.text = _expense.description!.toString();
    isEditing = true;
    setState(() {});
  }
  ...
  Future<void> handlerUpdate() async {
    ...
    Response response = await repository.update("${_expense.id}", data);
    if (response.statusCode == 200) {
      scaffoldMessengerKey.currentState?.showSnackBar(
        const SnackBar(
          content: Text("Pengeluaran telah diubah!"),
  ...
  void handlerDelete() async {
    ...
    final response = await repository.delete("${_expense.id}");
    if (response.statusCode == 200) {
      scaffoldMessengerKey.currentState?.showSnackBar(
        const SnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white,
      appBar: DetailHeader(title: _expense.code ?? "Detail Pengeluaran"), 
      body: Container(
                  ...
                  child: (isEditing) ? Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(_expense.code ?? "-", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                        ],
                      ),
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Text(_expense.tag.toString(), style: const TextStyle(fontSize: 12))
                              ],
                            ),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Text(_expense.name.toString(), style: const TextStyle(fontSize: 12))
                              ],
                            ),
                            const Divider(color: greyLightColor),
                            Row(
                              ...
                              children: [
                                ...
                                Text(parseRupiahCurrency("${_expense.nominal ?? 0}"), style: const TextStyle(fontSize: 12)),
                              ],
                            ),
                            const Divider(color: greyLightColor),
                            Row(
                              ...
                              children: [
                                ...
                                Text(formatDateFromString(_expense.createdAt ?? "", format: "dd/MM/yyyy"), style: const TextStyle(fontSize: 12)),
                              ],
                            ),
                            const Divider(color: greyLightColor),
                            Row(
                              ...
                              children: [
                                ...
                                Text(formatDateFromString(_expense.updatedAt ?? ""), style: const TextStyle(fontSize: 12)),
                              ],
                            ),
                          ],
                        ),
                      ...
                      Text("${_expense.description!.isNotEmpty ? _expense.description : "-"}", style: const TextStyle(fontSize: 10, color: greyTextColor)),
                    ],
                  ),
                ),
              )
