...
class _IncomesViewState extends State<IncomesView> {
  ...
  Future<void> handlerGetData() async {
    ...
    if (response.statusCode == 200) {
      incomes.clear();
      for (var item in response.data!["data"]!["incomes"]) {
        incomes.add(IncomeModel.fromJson(item));
      }
  ...
  void handlerLoadmore() async {
    ...
    if (res.statusCode == 200) {  
      for (var item in res.data!["data"]!["incomes"]) {
        incomes.add(IncomeModel.fromJson(item));
      }
  ...
  Future<void> deleteData(String id) async {
    ...
    if (response.statusCode == 200) {
      scaffoldMessengerKey.currentState?.showSnackBar(
        const SnackBar(
          content: Text("Pendapatan berhasil di hapus!"),
          backgroundColor: Colors.green,
        ),
      );
      handlerGetData();
      setState(() {
        incomes.removeWhere((element) => element.id.toString() == id);
      });
    } 
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("Apakah anda yakin ingin menghapus pendapatan ${incomes[i].code}?"),
          ...
          actions: [
            ...
            TouchableOpacity(
              onPress: () async {
                ...
                deleteData(incomes[i].id.toString());
              },
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
  ...
  Widget viewCard(BuildContext context, int index) {
    ...
    if (!loading && !loadmore && incomes.isEmpty)  {
      return const SizedBox(
        height: 40,
        child: Center(
          child: Text("Pendapatan tidak ditemukan!", style: TextStyle(color: greyTextColor, fontSize: 12)),
        ),
      );
    } 

    if (index == incomes.length && loadmore)  {
      return Center(
        child: Padding(
    ...
    return TouchableOpacity(
      onPress: () async {
        var result = await context.pushNamed(appRoutes.detailIncome.name, extra: jsonEncode(incomes[index]));
        if (result != null) {
          handlerGetData();
        }
      },
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(incomes[index].code ?? "-", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                    Text(incomes[index].createdAt!.isNotEmpty ? formatDateFromString(incomes[index].createdAt.toString(), format: "EEEE, dd/MM/yyyy") : "-", style: const TextStyle(fontSize: 8, color: greyTextColor)),
                  ],
                ),
                TouchableOpacity(
                  child: const Icon(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text(incomes[index].tag ?? "-", style: const TextStyle(fontSize: 8)),
                    ],
                  ),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text(incomes[index].name ?? "-", style: const TextStyle(fontSize: 8))
                    ],
                  ),
                  const Divider(color: greyLightColor),
                  Row(
                    ...
                    children: [
                      ...
                      Text(parseRupiahCurrency("${incomes[index].nominal ?? 0}"), style: const TextStyle(fontSize: 8)),
                    ],
                  ),
                ],
              ),
            ...
            child: Container(
              ...
              child: RefreshIndicator(
                ...
                child: ListView.builder(
                  ...
                  itemCount: loading ? 3 : loadmore ? incomes.length + 1 : incomes.isNotEmpty ? incomes.length : 1,
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                TouchableOpacity(
                  onPress: () async {
                    var result = await context.pushNamed(appRoutes.formIncome.name);
                    if (result != null) {
                      handlerGetData();
                    }
                  },
